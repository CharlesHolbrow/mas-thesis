...
\marginnote{Be aware of the diference between audio data compression
  ...
  thesis, \textit{compression} refers to \textit{dynamic range
    compression}.}
compressor is one of the most powerful and flexible tools in the audio
engineer's toolkit.  A basic compressor can be thought of as an
automatic volume control that simply reduces the level of an audio
signal when the signal exceeds a threshold. By carefully adjusting the
threshold, the input signals, the reduction amount, and the rate at
which the compressor reacts, a skilled audio engineer can use
...
expands on traditional audio compression model by adding a degree of
