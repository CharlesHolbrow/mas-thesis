The obvious next step is to make a fully featured surround compressor
with options for warping and attenuation, and parametric control of
width and spatial resolution of the regions to be attenuated. When a
region exeeds our threshold, we can choose exactly how much that
region is attenuated and how much it is warped. This would turn the
hypercompressor into a general tool that could be useful in a wide
variety of different situations. A more creative path would be adding
additional directional effects that can me modulated by the gain of
audio content in the surround image. For example when a region of the
surround sphere exceeds our threshold, we apply a sliding delay to
that region of sound image. We could choose from a selection of
different effects that can all be modulated by the positional gain of
the sound spere. We could even add spatial detection of other
qualities of the sound, and create a modulation matrix. For example we
detect only high frequency content around the surface of the sphere,
and and whenever a certain region of the sphere has more than a
certian level, we apply a phasing effect to that region. By
expanding on the paradigm of compression, we unlock 
