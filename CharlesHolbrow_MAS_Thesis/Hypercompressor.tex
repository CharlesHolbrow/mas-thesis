...
\item $N_n^{|m|}$ is a normalization factor.\sidenote{In ambisonic
    ...
    thesis asumes the SN3D convention as recommended by the AMBIX
...
\marginnote{The Hyper compressor uses SN3D normalization, and and ACN
  channel ordering. Both are described in \textit{AMBIX~-~A Suggested
    Ambisonics Format. C. Nachbar et al., 2011}} The spherical
...
\item With the cached spherical harmonics, it is then possible to
  ...
  rate.
\item A level meter does not need to update the display at the audio
  ...
  control rate, which could be as slow as 30 Hz. 
\end{enumerate}
...
\item Calculate a map of the influence of each lobe on the surround
  image. For example, pan a monophonic sound directly forward in an
  ambisonic mix, cache an image of the resulting sound sphere. Save
  one image for each of the 6 lobes.
...
%  \includegraphics{hypercomp/LevelDetect_45_down.png}
  \caption{The Hypercompressor visualizer written for the efficient
    ...
    cylinder, and unwrapped on the flat surface. In this image, a sine
    tone is panned slightly down and to the left ($45\degree$ azimuth,
    $-45\degree$ elevation) in front of the listener.}
  \label{fig:hypercomp-inf-map-angle}
\end{figure}
