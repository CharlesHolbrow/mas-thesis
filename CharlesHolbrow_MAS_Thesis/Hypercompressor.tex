...
By increasing the order of an ambisonic recording, we also increase
the number of audio channels required, and the precision which which
we can resolve position sounds in the recording. \TODO{Complete}

\subsection{Usage}
\label{sec:usage}
There are two ways to create an ambisonic recording: Use a soundfeild
microphone to recording an acoustic soundfield. \TODO{Complete}
...
Threshold and ratio parameters are essential for controling dynamic
...
\begin{itemize}
\item Perhaps\marginnote{The attack/release times listed here are a
    rough guide only. The effect of these parameters varies from one
    model of compressor to another. Results also depend on audio input
    material and threshold and ratio settings.} we want the compressor
  to engage or disengage at the time scale of a musical phrase. We
  could set our attack time long enough to let transients through
  without engaging the compressor significantly (try 15
  miliseconds). If our relsease time is quite long (try 300
  milliseconds), and we set our threshold and ratio carefully, we
  might be able to convince the compressor to smooth musical phrases.
...
\item If we reduce our attack and release parameters enough, we can
  ...
  your audio signal, adding odd order harmonics,\sidenote{Not every
...
\end{itemize}
Different compressors and compression settings offer different audio
character, quality. Compression is an especially intersteing signal
processing example, because in a musical context, the relationship
between the formula, and the feeling is comparatively well understood.
