...
itself. The attack time and realease time parameters give control over
this behavoir in the temporal domain. The next step is to
...
largely unganged from the the block-diagram for our traditional
...
\item Our audio signals must be updated to handle encoded
  amnbisonics. This is as simple as increasing the number of channels
...
\item On a traditional compressor, the level detector only needs to
  ...
  needs to decode the incoming signal, and and itentify both a threshold
  overage, and the region where the overage occured.
\item Our gain control module needs to listen to the input coming from
  the level detector module, and be able to attenuate the speciffic
  regions that exceede our threshold parameter.
\end{itemize}

\subsection{Ambisonic Matrix Calculations}
\label{sec:matrix-calculations}
\marginnote{The Hyper compressor uses SN3D normalization, and and ACN
  channel ordering. Both are described in \textit{AMBIX~-~A Suggested
    Ambisonics Format. C. Nachbar et al., 2011}}
The spherical harmonics defined in equation~\ref{eq:ambisonics} form a
set of orthoganal basis functions. If we define a squence for our
...
three-dimensional surround sound image.\cite{Kronlachner2014} 
The ability to mathematically warp and maniulate our makes ambisonics
the perfect choice for implementing a surround sound compressor. 

\subsection{Hypercompressor Level Detector Module}
...
to extract overage information with minimal ambisonic decoding.
\begin{enumerate}
\item To accurately play a first order ambisonic encoding, we need a
  ...
  detector, we calculate the root mean square (RMS) average at the center of
  6 lobes corespoinging to the first order spherical harmonics; front,
  rear, left, right, top, and bottom.
\item Calculate a map of the influce of each lobe on the surround image. For
  example, pan a monophonic sound directly foward in an ambisonic
...
\item We have 6 images, each representing one of the 6 lobes of our
  ...
  maps. The sum of the weighted maps show the gain distributed accross
...
\begin{figure}[h]
  ...
  \caption{Calculating the influcene in Wolfram Mathematica software
    package}
  \label{fig:hypercomp-inf-map-angle}
\end{figure}


%\paragraph{Results} While not completely mathematically rigerous, this
%approach worked very well in certain contexts.

\TODO{Finish describing Hypercompression Development. Document
